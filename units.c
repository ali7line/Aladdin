 ...
 *                                                                    
 *  Written  by: M.A. Austin, Xiaoguang Chen, and Wane-Jang Lin     December 1995
 *  ============================================================================= 
 */
...
int            length;
    ...
    if( d1==(DIMENSIONS *)NULL || d2==(DIMENSIONS *)NULL ) {
        printf("Warning: d1 or d2 is NULL,   in UnitsMultRep()\n");
...
#endif
{
 ...
 switch(units_type) {
   case SI:
    ...
    if(eng_units.units_name != (char *)NULL && !strcmp("Jou", eng_units.units_name))
       eng_units.scale_factor = 1.0/0.4535924/0.0254/9.80665;
    else if(eng_units.units_name != (char *)NULL && !strcmp("kJ", eng_units.units_name))
       eng_units.scale_factor = 1E+3/0.4535924/0.0254/9.80665;

    /* [f] incremental temperature units */

    if(eng_units.units_name != (char *)NULL && !strcmp("DEG_C", eng_units.units_name))
       eng_units.scale_factor = 9.0;

    break;
    default:
      printf(" UNITS_TYPE = %d \n", (int) units_type);
      FatalError("Units_type must be SI or US to use this function", (char *) NULL);
    break;
